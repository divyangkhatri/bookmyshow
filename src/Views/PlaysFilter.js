...
const PlaysFilter = ({navigation, route}) => {
  ...
  React.useEffect(() => {
    if (
      ...
      setLanguageSelected(route.params?.applyLanguage);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (
      (categorySelected !== route.params?.applyCategory ||
        ...
        languageSelected !== route.params?.applyLanguage ||
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [categorySelected, dateSelected, selected, paramKey, languageSelected]);
  ...
  const handleCellClick = (item, section) => {
    if (item.name === 'Show All' || item.name === 'Show Less') {
      ...
      if (selected && selected.indexOf(item.name) !== -1) {
        ...
        languageSelected &&
        languageSelected.indexOf(item.name) !== -1
      ) {
        setLanguageSelected(
          languageSelected.filter((items) => item.name !== items),
        );
        ...
        if (section.key === 'DATE' && item.name !== 'Date Range') {
          ...
          setLanguageSelected([...languageSelected, item.name]);
  ...
  const tempDataFilter = () => {
    ...
    if (!isShowMore) {
      tempData[1].data.splice(3, 4);
      tempData[1].data.push({name: 'Show All'});
    } else {
      tempData[1].data.push({name: 'Show Less'});
    }
  };
  tempDataFilter();
    ...
    <SafeAreaView
      ...
      <View style={navigationView}>
        <View style={navigationCornerView}>
          <TouchableOpacity
            onPress={() => {
              navigation.navigate('PlaysScreen');
            }}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              navigation.navigate('PlaysScreen', {
                ...
                applyDate: dateSelected ? dateSelected : new Date(dateChange),
                applyPrice: selected,
