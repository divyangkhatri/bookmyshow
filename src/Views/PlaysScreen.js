...
const PlaysScreen = ({navigation, route}) => {
  const {
    safeAreaView,
    mainView,
    categoryText,
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      title: 'Plays in ' + route.params.selectedCity,
      headerRight: () => (
        <TouchableOpacity>
          <Image
  ...
  React.useEffect(() => {
    ...
    if (route.params?.applyLanguage) {
      setLanguage(route.params?.applyLanguage);
    } else {
      setApplyPrice([]);
    }
...
const FlatListView = (prop) => {
  ...
  const data = [
    {
      key: '201',
      name: 'Theater Live',
      category: ['Online Streaming Plays', 'Storyteller'],
      languages: ['English', 'Hindi'],
      date: new Date(2020, 4, 6),
      venue: 'Online Streaming',
      image: require('../assets/images/theaterLive.jpg'),
      isOnwards: true,
      price: 0,
    },
    {
      key: '202',
      name: 'Suroon ka karwaan',
      languages: ['Hindi'],
      category: ['Musical'],
      date: new Date(2020, 4, 17),
      venue: 'Sanjeev Kumar Auditorium, Adajan ,Surat',
      image: require('../assets/images/suroor.jpg'),
      isOnwards: true,
      price: 700,
    },
    {
      key: '203',
      name: 'Sir Sir Sarla',
      image: require('../assets/images/sarla.jpg'),
      category: ['Comedy'],
      languages: ['Gujarati'],
    ...
    },
    {
      ...
      languages: ['Gujarati'],
      image: require('../assets/images/Natak.jpg'),
      category: ['Comedy'],
  ...
  if (
      ...
      tempData = data.filter((item) => {
        if (
          ...
          return (
            ...
            item.languages.some((r) => prop.language.indexOf(r) >= 0)
          );
        } else if (
          prop.price.indexOf('501 - 2000') !== -1 &&
          prop.price.indexOf('Above 2000') !== -1
    ...
    } else if (prop.language.length > 0 && prop.category && prop.date) {
      tempData = data.filter((item) => {
        return (
          item.date.getTime() === dt.getTime() &&
          item.category.indexOf(prop.category) !== -1 &&
    ...
    } else if (prop.price.length > 0 && prop.category && prop.date) {
      tempData = data.filter((item) => {
        if (
          prop.price.indexOf('501 - 2000') !== -1 &&
          prop.price.indexOf('Above 2000') !== -1 &&
    ...
    } else if (prop.price.length > 0 && prop.date && prop.language.length > 0) {
      tempData = data.filter((item) => {
        if (
          prop.price.indexOf('501 - 2000') !== -1 &&
          prop.price.indexOf('Above 2000') !== -1 &&
    ...
    } else if (prop.price.length > 0 && prop.language.length > 0) {
      tempData = data.filter((item) => {
        if (
          prop.price.indexOf('501 - 2000') !== -1 &&
          prop.price.indexOf('Above 2000') !== -1 &&
          ...
          return item.languages.some((r) => prop.language.indexOf(r) >= 0);
        } else if (
          prop.price.indexOf('501 - 2000') !== -1 &&
          prop.price.indexOf('Above 2000') !== -1
        ) {
          ...
          return (
            item.languages
              .some((r) => prop.language.indexOf(r) >= 0)
              .getTime() === dt.getTime() && item.price > 2000
          );
        }
      });
    ...
    } else if (prop.category && prop.language.length > 0) {
      tempData = data.filter(
        (item) =>
          item.category.indexOf(prop.category) !== -1 &&
          item.languages.some((r) => prop.language.indexOf(r) >= 0),
      );
    } else if (prop.date && prop.language.length > 0) {
      tempData = data.filter(
        (item) =>
          item.date.getTime() === dt.getTime() &&
          item.languages.some((r) => prop.language.indexOf(r) >= 0),
    ...
    } else if (prop.language.length > 0) {
      tempData = data.filter((item) =>
        item.languages.some((r) => prop.language.indexOf(r) >= 0),
      );
    }
    ...
    return (
      <View
        ...
        <Text style={categoryText} numberOfLines={1}>
          {items.item.languages + ', ' + items.item.category.toString()}
        </Text>
        {/*</View>*/}
...
const HeaderView = (navigation) => {
  ...
  return (
    <View style={mainView}>
      <TouchableWithoutFeedback
        onPress={() =>
          navigation.navigate('MapViewScreen', {plays: 'Plays Venue'})
        }>
